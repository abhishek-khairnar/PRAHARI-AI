...
print("=" * 60)
print("STAGE-BY-STAGE LATENCY PROFILER")
print("=" * 60)
...
if not ret:
    print("Failed to capture RTSP frame!")
    sys.exit(1)
print(f"Captured RTSP Frame: {test_frame.shape}")
...
# Warmup GPU
for _ in range(10):
    _ = model.predict(source=test_frame, imgsz=640, device="cuda", half=True, verbose=False)
...
# Run 100 iterations
N = 100
t_cap_list, t_res_list, t_yolo_list, t_track_list, t_face_list, t_draw_list, t_enc_list = [], [], [], [], [], [], []
...
for i in range(N):
    ...
    # 5. Face Detection (Global 640x360 once every 5 frames)
    t0 = time.perf_counter()
    if i % 5 == 0:
...
print("\n" + "=" * 60)
print("STAGE TIMING BENCHMARK (Averages across 100 frames)")
print("=" * 60)
print(f"RTSP Capture:       {avg_cap:6.2f} ms")
print(f"Frame Resize:       {avg_res:6.2f} ms")
print(f"YOLO Inference:     {avg_yolo:6.2f} ms (GPU FP16 on RTX 3050)")
